...
export default function Categories() {
  ...
  return (
    <View style={{ marginTop: 20 }}>
        ...
        {categories.map((category) => {
          ...
          return (
            <View key={category._id} style={{ marginRight: 10 }}>
              ...
              <Text style={{ marginTop: 5, fontSize: 14, ...themeColors, ...textClass }}>{category.name}</Text>
            </View>
          );
        })}
      </ScrollView>
      ...
      {dishes.map((dish) => (
        <TouchableOpacity key={dish._id} onPress={() => navigateToDishDetail(dish._id)}>
          <View style={{ marginBottom: 20 }}>
            ...
            <View style={{ padding: 10 }}>
              ...
              <Text style={{ marginTop: 5, fontSize: 14, color: '#888' }}>{dish.description}</Text>
